...
def parameters():
    # parameters:
    global tau, D_diff, cp_cooling, avg_Ts, amp_Ts, lifetime_rain, interaction, persistence_boost
    ...
    tau = 3.6e4 #* (10 hours) unit: s, coefficient that determines how the T_ns is modulated by the T_s #0.05
    D_diff = 1e2 #* 1e2 # unit: m**2/s, coefficient that determines how quickly the field is 'diffused'  #0.8 # 0.4 #0.6
    cp_cooling = 2/20 #2/20 #* unit: 20 K
    avg_Ts = 0.5 # unit: 20 K
    amp_Ts = 0.5 # unit: 20 K
    lifetime_rain = 2 # unit: time steps that rain lasts and can interact
    interaction = 0.7 #0.7 #scale the interaction coefficient
